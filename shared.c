...
void parse_config() {
	...
	get_bool(ENABLE_SINGLEPLAYER_AS_INSECURE);

	get_string(HYTALE_COM);
...
int execve(const char* filename, char* const argv[], char* const wenvp[]) {
	...
	if (should_modify_argument(program) == 0) {
	...
	// run modify_argument on all arguments ..
	for (int i = 0; i < argc; i++) {
		int keep = modify_argument(program, argv[i]);
		if (keep == 1) {
	...
	// run modify_argument on all environment variabless ..
	for (int i = 0; i < wenvc; i++) {
		int keep = modify_argument(program, wenvp[i]);
		if (keep == 1) {
...
BOOL WINAPI CreateProcessW_hook(
	...
	// check each argument against modify_argument.
	for (int i = 0; i < wargc; i++) {
		...
		int keep = modify_argument(program, arg);
	...
	if (lpEnvironment != NULL) {
		...
		for (size_t i = 0; wcscmp(&wenv[i], L"") != 0; i += wcslen(&wenv[i]) + 1) {
			...
			int keep = modify_argument(program, n_env);
